...
class CustomerController extends Controller
{
    ...
    public function searchIndex(Request $request) {
        ...
        $productAvail = t_food::where("name", "LIKE", "%$searchedProduct%")->where('del_stat', 1)->first();
        $product = t_food::where("name", "LIKE", "%$searchedProduct%")->where('del_stat', 1)->get();    
    ...
    public function searchDrink(Request $request) {
        ...
        $productAvail = t_drink::where("name", "LIKE", "%$searchedProduct%")->where('del_stat', 1)->first();
        $product = t_drink::where("name", "LIKE", "%$searchedProduct%")->where('del_stat', 1)->get();
        if($productAvail) {
